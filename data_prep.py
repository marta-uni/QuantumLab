...
folder_name = 'data6'
file_paths = [f"{folder_name}/scope_{i}.csv" for i in range(15, 26)]  # Example: process files scope_24.csv to scope_25.csv
os.makedirs(f"{folder_name}/figures", exist_ok=True)

# Loop through each file in the file_paths
for file_path in file_paths:
    file_name = os.path.basename(file_path).replace('.csv', '')  # Extract file name without extension
    ...
    output_file = figure_name = f'{folder_name}/figures/{file_name}_cropped.csv'  # Adjust the file path as needed
    fn.save_to_csv(timestamps, volt_laser, volt_piezo, piezo_fitted, output_file)
