...
def transmission_temp_no_off_original(x, ampl, scale1, scale2, temp):
    kb = 1.38e-23  # J/K
...
def transmission_temp_no_off(x, ampl, scale1, scale2, temp):
    ...
    c = 3e8
    term2 = kb / (c**2 * rb_mass)
    return ampl * (np.exp(- (scale1 * np.exp(-(x - peaks['freq'][3])**2 / (2 * term2 * (peaks['freq'][3]**2) * temp)) +
                                                 scale2 * np.exp(-(x - peaks['freq'][4])**2 / (2 * term2 * (peaks['freq'][4]**2) * temp)))) - 1)

data = pd.read_csv('data9/clean_data/Fixed_ld00000_frequencies_cropped_3.csv')
...
popt, pcov = curve_fit(transmission_temp_no_off_original, xdata=restricted_freq,
...
pd_fit = transmission_temp_no_off_original(f, *popt)
...
plt.legend()
plt.tight_layout()
plt.savefig('data9/figures/temperature/temp_fit_quad.pdf')
plt.show()
